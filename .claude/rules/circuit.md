# Circuit Reference — Echo Pre 3 (Echoplex EP-3 preamp)  (TEMPLATE — fill in from the schematic)
...
Target circuit: the Echoplex EP-3 tube preamp, traced via the **Chase Tone Secret Preamp**
schematic. Drop the schematic image(s) into `schematics/` and list them below.
...
| File | Role |
|------|------|
| `<chase-tone-secret-preamp>.png` | Primary source of truth for values + topology (Chase Tone Secret Preamp schematic) |
| `<switch_ref>.jpg` | Authoritative for switch/diode network, if separate |
