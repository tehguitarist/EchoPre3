...
| `imd_and_floors.py` | ~150 | Reads the three things in the capture set nothing else touches: the **noise floor**, the **repeatability floor** (a byte-identical duplicate cell, so a memoryless system returns −inf and anything above is the capture's own limit), and the **twin-tone segment**. ⚠ That segment is 220 + 660 Hz and 660 = 3 × 220 exactly, so every intermodulation product lands on the 220 Hz harmonic grid and none is separable from harmonic distortion — it cannot measure IMD, and an inharmonic pair would be needed to fix that. ⭐ What it can do is an **off-grid known-answer probe**: a memoryless circuit can put energy only on that grid, so whatever a capture shows at half-integer multiples is its own error floor for the segment, measured with no reference. |
